...
import { STATUS_READY, STATUS_SENT } from '../models/crypts.js';
...
export default async function main() {
  ...
  await pMap(cryptsToNotify, async (crypt) => {
    ...
    const email = {
      ...
      html: `
<p>Dear ${crypt.to_name}<br>${crypt.from_name} wrote a message some time ago. They scheduled it to be sent after some time in the event they stopped online activity.<br />We tried to reach them multiple times over the last month, and were unable to get any update from them, so we're now assuming the worst and releasing the content he trusted us with.</p>
